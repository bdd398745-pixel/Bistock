...
def max_allowed_period(interval: str) -> str:
    ...
    if i in {"2m","5m","15m","30m"}:
        return "30d"    # 5m/15m/30m: practical cap around ~30 days
    if i in {"60m","90m","1h"}:
...
def compute_min_indicators(long_df: pd.DataFrame) -> pd.DataFrame:
    def calc_group(g: pd.DataFrame) -> pd.DataFrame:
        ...
        # RSI14 (simple Wilder approximation)
...
def fast_score(row: pd.Series) -> float:
    ...
    # Minimal components (no vol proxy, no slope):
...
if pro_orb:
    # Recompute OR levels just for latest session quickly
...
st.download_button("⬇️ Download Latest Signals (CSV)", data=csv_data, file_name=f"fastscan_signals_{datetime.now(IST).strftime('%Y%m%d_%H%M')}.csv", mime="text/csv")
...
if chart_ticker:
    ...
    if hist.empty:
        ...
        # OR for latest session if present
